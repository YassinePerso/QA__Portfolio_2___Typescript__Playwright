...
test.describe('Informations de paiement', () => {
  ...
  // TC-012 - Vérifier la présence des champs Account name et Account number
  test('TC-012 - Vérifier la présence des champs Account name et Account number', { tag: '@smoke' }, async () => {
    await expect(checkoutPage.accountNameInput).toBeVisible();
    await expect(checkoutPage.accountNumberInput).toBeVisible();
  });

  // TC-013 - Champs vides
  test('TC-013 - Champs vides', { tag: '@regression' }, async () => {
    await expect(checkoutPage.finishButton).toBeDisabled();
  });

  // TC-014 - Champs remplis (accès à la confirmation)
  test('TC-014 - Champs remplis (accès à la confirmation)', { tag: '@smoke' }, async () => {
    ...
    await expect(checkoutPage.paymentSuccessMessage).toBeVisible();     // Vérifie l'accès à la confirmation
  });
